...
from Simulador import TIEMPOS_PROMEDIO

# Parámetros de la simulación
RANDOM_SEED = 44
MEMORIA_RAM = 100
VELOCIDAD_CPU = 4  # Número de instrucciones por unidad de tiempo
...
def proceso(env, nombre, memoria, cpu, tiempo_total, io_list):
    ...
    # Generar un número aleatorio para la memoria
...
def ejecutar_simulacion(env, num_procesos, tiempo_simulacion, intervalo_llegada):
    memoria = sympy.Container(env, init=MEMORIA_RAM, capacity=MEMORIA_RAM)
    cpu = sympy.Resource(env, capacity=1)
...
def ejecutar_C():
    ...
    for nombre_estrategia, memoria_estrategia, velocidad_estrategia, num_cpus_estrategia in ESTRATEGIAS:
        ...
        for num_procesos in NUM_PROCESOS:
            env = sympy.Environment()
            memoria = sympy.Container(env, init=memoria_estrategia, capacity=memoria_estrategia)
            cpu = sympy.Resource(env, capacity=num_cpus_estrategia)
            tiempo_promedio_iteracion = []
            for _ in range(5):
                env.process(ejecutar_simulacion(env, num_procesos, TIEMPO_MAXIMO, INTERVALOS_LLEGADA[0]))  # Intervalo de llegada de 10
                env.run()  # Ejecutar la simulación
        ...
        # Proceso para graficar resultados para la estrategia al momento
